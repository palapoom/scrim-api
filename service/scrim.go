...
func ScrimGetOffer(teamId string) (*model.ScrimGet, error) {
	...
	rows, err = database.Db.Query("SELECT scrim.scrim_id, scrim_offer.team_id, t.team_logo, t.team_name, scrim.scrim_map, scrim.scrim_date, scrim.scrim_time FROM scrim INNER JOIN scrim_offer ON scrim.scrim_id = scrim_offer.scrim_id INNER JOIN team AS t ON scrim_offer.team_id = t.team_id WHERE scrim.team_id = $1 AND offer_status = 'pending' AND (scrim.scrim_date > CURRENT_DATE OR (scrim.scrim_date = CURRENT_DATE  AND scrim.scrim_time > CURRENT_TIME )) ORDER BY scrim.scrim_date ASC, scrim.scrim_time ASC;", teamId)
	if err != nil {
		return nil, err
	}
